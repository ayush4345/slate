...
Demo **provider agent**. Sells four keyless services per call through a
...
| `TransactionPreflightService` | Base RPC (`eth_call` + `eth_estimateGas`) | `{ chainId, from, to, data?, value? }` → simulation status, gas estimate, decoded calldata, safety verdict |

All four sit behind one `ToolboxService`, so a consumer can mix tools and
